<div class="min-h-full w-full flex-1 bg-[#f6f7f2] p-4 text-[#17221b] lg:p-8 dark:bg-zinc-950 dark:text-zinc-100">
            ...
            <div class="overflow-x-auto">
                <flux:table :paginate="$this->users" class="min-w-[960px] text-sm">
                    ...
                    <flux:table.rows>
                        ...
                        @empty
                        <flux:table.row>
                            <flux:table.cell colspan="6" class="py-12 text-center text-[#637168]">No users match your search.</flux:table.cell>
                        </flux:table.row>
                        @endforelse
                    </flux:table.rows>
                </flux:table>
    ...
    <flux:modal name="create-user" wire:model.self="showCreateForm" class="md:w-[min(720px,calc(100vw-2rem))]">
        <form wire:submit="createUser" class="space-y-5">
            <div>
                <flux:heading size="lg">Create workspace user</flux:heading>
                <flux:subheading class="mt-1">Choose whether credentials should be emailed or handed over manually.</flux:subheading>
            </div>
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <flux:input wire:model="position" label="Department / position" placeholder="Procurement Officer" required />
                <flux:select wire:model="role" label="System role">
                    <option value="officer">Procurement officer</option>
                    <option value="finance">Finance</option>
                    <option value="admin">Company admin</option>
                </flux:select>
                <flux:select wire:model="deliveryChannel" label="Credential delivery">
                    <option value="email">Send temporary password by email</option>
                    <option value="manual">Set credentials manually</option>
                </flux:select>@if($deliveryChannel === 'manual')
                <flux:input wire:model="password" type="password" label="Temporary password" class="md:col-span-2" required />@endif
            </div>
            <div class="flex flex-wrap items-center gap-4">
                <flux:checkbox wire:model="isActive" label="Activate account immediately" />
                <flux:checkbox wire:model="forcePasswordChange" label="Force password change on first login" />
            </div>
            <div class="flex justify-end gap-3">
                <flux:button type="button" wire:click="$set('showCreateForm', false)" variant="ghost">Cancel</flux:button>
